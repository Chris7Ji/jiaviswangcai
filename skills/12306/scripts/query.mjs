...
const F = {
  ...
  departTime: 8, arriveTime: 9, duration: 10, canBuy: 11,
  date: 13, gr: 21, rw: 23, rz: 24, tz: 25, wz: 26,
  yw: 28, yz: 29, ze: 30, zy: 31, swz: 32, dw: 33,
};
...
Options:
  ...
  --seat <types> Only show trains with tickets for given seats (comma-separated: swz,zy,ze,rw,dw,yw,yz,wz)
...
function parseTicket(raw, stationMap) {
  ...
  return {
    ...
    gr: v('gr'), rw: v('rw'), dw: v('dw'), yw: v('yw'),
    rz: v('rz'), yz: v('yz'), wz: v('wz'),
  };
}
...
<tr>
  ...
  const filterTag = filterDesc ? `<div class="filters">${e(filterDesc)}</div>` : '';
  return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>${fn} → ${tn} 列车的刻表</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .type-g { color: #0071e3; } .type-d { color: #34c759; } .type-z { color: #af52de; }
    .type-t { color: #ff9500; } .type-k { color: #86868b; }
    .time { font-variant-numeric: tabular-nums; }
    .depart { font-weight: 600; }
    .arrow { margin: 0 4px; color: #c0c0c0; }
    .arrive { color: #6e6e73; }
    .duration { color: #86868b; font-variant-numeric: tabular-nums; }
    .na { color: #d2d2d7; } .available { color: #34c759; font-weight: 500; }
    .sold-out { color: #ff3b30; } .count { font-weight: 600; font-variant-numeric: tabular-nums; }
    .buy-yes { color: #34c759; font-weight: 500; } .buy-no { color: #ff3b30; font-weight: 500; }
...
<body>
  <div class="container">
    <header>
      ...
      <div class="meta"><span>${e(travelDate)}</span><span>${tickets.length} 列车</span></div>
      ${filterTag}
    </header>
...
function buildMarkdown(tickets, from, to, travelDate, filterDesc) {
  const lines = [];
  lines.push(`## ${from.station_name} → ${to.station_name} | ${travelDate} | ${tickets.length} 列车`);
...
if (values.json) {
  ...
  const outPath = values.output || join(__dirname, '..', 'data', `${fromStation.station_name}-${toStation.station_name}-${date}.html`);
