...
LOG_FILE="/Users/jiyingguo/.openclaw/workspace/install_retry_v3.log"
RESULT_FILE="/Users/jiyingguo/.openclaw/workspace/install_results_v3.json"

# 所有待安装技能（包括新加的multi-search-engine）
SKILLS=("joke-teller" "openclaw-tavily-search" "multi-search-engine")
SCHEDULE=("09:40" "10:30" "11:30" "14:30" "17:30")

# 初始化日志文件
echo "=== 技能安装重试日志 v3 $(date) ===" > "$LOG_FILE"
echo "[" > "$RESULT_FILE"
...
# 显示要安装的技能
echo "" | tee -a "$LOG_FILE"
echo "📋 本次重试安装的技能:" | tee -a "$LOG_FILE"
for i in "${!SKILLS[@]}"; do
    echo "$((i+1)). ${SKILLS[$i]}" | tee -a "$LOG_FILE"
done
...
install_skill() {
    ...
    case "$skill" in
        "openclaw-tavily-search")
            echo "[$timestamp] 同时尝试安装tavily-tool作为备选..." | tee -a "$LOG_FILE"
            ;;
        "multi-search-engine")
            echo "[$timestamp] 同时尝试安装multi-search-engine-2作为备选..." | tee -a "$LOG_FILE"
            ;;
    esac
    ...
    if [ $exit_code -ne 0 ]; then
        case "$skill" in
            "openclaw-tavily-search")
                echo "[$timestamp] 尝试备选技能: tavily-tool" | tee -a "$LOG_FILE"
                local alt_output=$(clawhub install "tavily-tool" 2>&1)
                local alt_exit_code=$?
                
                if [ $alt_exit_code -eq 0 ]; then
                    output="$alt_output"
                    exit_code=0
                    skill="tavily-tool"  # 更新技能名称为实际安装的
                    echo "[$timestamp] ✓ 成功安装备选技能: tavily-tool" | tee -a "$LOG_FILE"
                fi
                ;;
            "multi-search-engine")
                echo "[$timestamp] 尝试备选技能: multi-search-engine-2" | tee -a "$LOG_FILE"
                local alt_output=$(clawhub install "multi-search-engine-2" 2>&1)
                local alt_exit_code=$?
                
                if [ $alt_exit_code -eq 0 ]; then
                    output="$alt_output"
                    exit_code=0
                    skill="multi-search-engine-2"  # 更新技能名称为实际安装的
                    echo "[$timestamp] ✓ 成功安装备选技能: multi-search-engine-2" | tee -a "$LOG_FILE"
                fi
                ;;
        esac
    fi
...
if [ $success_count -gt 0 ]; then
    echo "" | tee -a "$LOG_FILE"
    echo "📢 生成安装成功通知..." | tee -a "$LOG_FILE"
    
    NOTIFICATION_FILE="/Users/jiyingguo/.openclaw/workspace/notification_v3.txt"
    echo "🎉 技能安装成功通知 (v3)" > "$NOTIFICATION_FILE"
    echo "时间: $(date)" >> "$NOTIFICATION_FILE"
    echo "" >> "$NOTIFICATION_FILE"
    echo "成功安装的技能:" >> "$NOTIFICATION_FILE"
    
    # 从结果文件中提取成功安装的技能
    if [ -f "$RESULT_FILE" ]; then
        grep '"success": true' "$RESULT_FILE" | grep -o '"skill": "[^"]*"' | cut -d'"' -f4 | while read skill; do
            echo "- $skill" >> "$NOTIFICATION_FILE"
        done
    fi
    
    echo "" >> "$NOTIFICATION_FILE"
    echo "详细结果请查看: $RESULT_FILE" >> "$NOTIFICATION_FILE"
    echo "完整日志请查看: $LOG_FILE" >> "$NOTIFICATION_FILE"
    
    echo "通知文件已生成: $NOTIFICATION_FILE" | tee -a "$LOG_FILE"
fi
