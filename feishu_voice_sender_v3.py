...
def send_audio_message(token, file_key):
    """发送音频消息"""
    # 使用audio类型
    content = json.dumps({
        "file_key": file_key
...
def send_voice(file_path):
    ...
    # 上传文件
    file_key = upload_audio(token, file_path)
    if not file_key:
        return False
    print(f"✅ 文件上传成功: {file_key}")
    
    # 发送消息
    if send_audio_message(token, file_key):
        print("✅ 语音消息发送成功！")
        return True
    return False
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python feishu_voice_sender_v3.py <音频文件路径>")
        sys.exit(1)
