...
def upload_voice(token, file_path):
    """上传语音文件到飞书 - 使用voice类型"""
...
def send_voice_message(token, file_key, duration=30000):
    """发送语音消息"""
    content = json.dumps({
        "file_key": file_key,
        "duration": duration
...
def send_voice(file_path):
    ...
    # 上传文件
    file_key = upload_voice(token, file_path)
    if not file_key:
        return False
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python feishu_voice_sender_v2.py <音频文件路径>")
        sys.exit(1)
