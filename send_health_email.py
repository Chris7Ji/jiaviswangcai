...
def create_email_content(md_content):
    ...
    msg['Subject'] = f'健康长寿科研成果监控报告 - {today}'
    ...
    <body>
        <div style="max-width: 800px; margin: 0 auto; padding: 20px;">
            {md_content.replace(chr(10), '<br>').replace('# ', '<h1>').replace('## ', '<h2>').replace('### ', '<h3>')}
            <div class="footer">
                <p>此邮件由OpenClaw自动生成</p>
...
def main():
    ...
    if not report_file:
        # 默认查找今天的文件
        report_file = f"/Users/jiyingguo/.openclaw/workspace/健康长寿科研成果监控报告_{today}.md"
    
    print("=" * 60)
    print("健康长寿科研成果监控报告邮件发送系统")
    print("=" * 60)
    ...
    # 读取文件内容
    print("\n📖 正在读取健康报告文件...")
    md_content = read_file(report_file)
    
    if not md_content:
        print("❌ 错误: 无法读取文件内容")
        sys.exit(1)
    
    print(f"✅ 报告文件: {len(md_content)} 字符")
    
    # 创建邮件
    print("\n📧 正在创建邮件...")
    msg = create_email_content(md_content)
