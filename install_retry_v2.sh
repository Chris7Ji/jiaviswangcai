...
# 安装重试脚本 v2
# 按照时间表重试安装games、joke-teller和tavily-search技能
...
SKILLS=("games" "joke-teller" "openclaw-tavily-search")
SCHEDULE=("09:30" "10:30" "11:30" "14:30" "17:30")

# 初始化日志文件
echo "=== 技能安装重试日志 v2 $(date) ===" > "$LOG_FILE"
echo "[" > "$RESULT_FILE"
...
install_skill() {
    ...
    # 特殊处理：对于tavily-tool也尝试安装
    if [ "$skill" == "openclaw-tavily-search" ]; then
    ...
    # 如果主技能失败，尝试备选技能（针对tavily-search）
    if [ $exit_code -ne 0 ] && [ "$skill" == "openclaw-tavily-search" ]; then
...
if [ $success_count -gt 0 ]; then
    ...
    NOTIFICATION_FILE="/Users/jiyingguo/.openclaw/workspace/notification.txt"
    echo "🎉 技能安装成功通知" > "$NOTIFICATION_FILE"
