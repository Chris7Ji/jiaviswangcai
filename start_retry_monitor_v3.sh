...
# 监控脚本 v3 - 启动安装重试并监控进度（包含multi-search-engine）

echo "🚀 启动技能安装重试监控系统 v3"
echo "=========================================="
...
if ps aux | grep "install_retry_v3.sh" | grep -v grep > /dev/null; then
    ...
    pkill -f "install_retry_v3.sh"
    sleep 2
fi

# 设置日志文件
MONITOR_LOG="/Users/jiyingguo/.openclaw/workspace/monitor_output_v3.log"
echo "=== 监控启动时间: $(date) ===" > "$MONITOR_LOG"

# 显示要安装的技能
echo "" | tee -a "$MONITOR_LOG"
echo "📋 本次重试安装的技能:" | tee -a "$MONITOR_LOG"
echo "1. joke-teller - 笑话生成器" | tee -a "$MONITOR_LOG"
echo "2. openclaw-tavily-search - Tavily搜索 (备选: tavily-tool)" | tee -a "$MONITOR_LOG"
echo "3. multi-search-engine - 多搜索引擎 (备选: multi-search-engine-2)" | tee -a "$MONITOR_LOG"

# 显示执行计划
echo "" | tee -a "$MONITOR_LOG"
echo "⏰ 执行计划时间表:" | tee -a "$MONITOR_LOG"
echo "09:40 - 第一次尝试" | tee -a "$MONITOR_LOG"
...
nohup bash /Users/jiyingguo/.openclaw/workspace/install_retry_v3.sh >> "$MONITOR_LOG" 2>&1 &
RETRY_PID=$!

echo "✅ 重试脚本已启动 (PID: $RETRY_PID)" | tee -a "$MONITOR_LOG"
echo "📁 监控日志: $MONITOR_LOG" | tee -a "$MONITOR_LOG"
echo "📁 安装日志: /Users/jiyingguo/.openclaw/workspace/install_retry_v3.log" | tee -a "$MONITOR_LOG"
echo "📁 结果文件: /Users/jiyingguo/.openclaw/workspace/install_results_v3.json" | tee -a "$MONITOR_LOG"
...
echo "- 下一个计划时间: 09:40" | tee -a "$MONITOR_LOG"

# 创建状态检查脚本
cat > /Users/jiyingguo/.openclaw/workspace/check_retry_status_v3.sh << 'EOF'
#!/bin/bash
...
chmod +x /Users/jiyingguo/.openclaw/workspace/check_retry_status_v3.sh

echo "" | tee -a "$MONITOR_LOG"
echo "📝 状态检查命令:" | tee -a "$MONITOR_LOG"
echo "bash /Users/jiyingguo/.openclaw/workspace/check_retry_status_v3.sh" | tee -a "$MONITOR_LOG"
...
bash /Users/jiyingguo/.openclaw/workspace/check_retry_status_v3.sh | tee -a "$MONITOR_LOG"
